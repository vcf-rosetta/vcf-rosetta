...
> ⏸️ **状态:暂缓的 POC,默认不部署。** 界面汉化已由 [`../browser-extension/`](../browser-extension/) 完整覆盖;
> 本插件唯一的增量价值在"中文运维**语义层**"(告警解释等),目前还薄(`semantics/` 仅 ~18 条种子),
> 且部署成本高(服务器跑服务 + 证书 + 注册 vCenter)。**当前出货只需扩展**;等语义层做厚再考虑启用。
> 下面的部署文档**仅作 POC 留档**。
>
> 注意:`i18n/`(术语词库)**不随本插件暂缓** —— 它是扩展的活跃词库源,见 [`i18n/README.md`](i18n/README.md)。
...
| 术语查询 | 英↔中检索(取自 `i18n/`,zh-CN 现 49,950 条) | 内置词表(离线) |
