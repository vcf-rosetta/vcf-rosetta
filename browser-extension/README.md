...
**怎么确认成功**:打开弹窗看「词典」行的**条数**——变成最新条数(如 52,193)即已更新。

> 商店版 / 轻量包用户不需要 build:他们的词典从 CDN 按需下载。3.4.32 起扩展会在每次页面加载时
> 检查 `@main` 上的 `langs.json` 目录 —— 维护者 `publish-langpacks.sh` 发布(bump version)后,
> 已装用户**下次打开 vCenter 页即自动拉到新词典**,无需手动刷新(弹窗 **↻ 刷新词典** 仍可立即强制重取)。
> 本节只针对**开发者模式/离线包**(词典内置)的更新。
...
> [Releases](https://github.com/vcf-rosetta/vcf-rosetta/releases/latest) 页**只提供离线包**(按版本命名的
> `vcf-rosetta-<版本>-offline.zip` + 固定名副本,均词典内置)——不再附带轻量包,避免用户下错不含词典的包;
> 轻量版 = Chrome 应用商店渠道(`pack-store.mjs` 产出的 lite zip 仅用于商店后台上传)。
...
# ★ Release 只传离线包(词典内置)—— 轻量包仅用于商店后台上传,不随 Release 分发
gh release create v<版本> browser-extension/dist/vcf-rosetta-<版本>-offline.zip -t "v<版本>" -n "离线包(词典内置)"
