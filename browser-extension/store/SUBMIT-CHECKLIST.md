...
## 1. 先发布语言包,再构建上传包
扩展包**不内置词典**(仅 28KB),运行时按需从 `langpacks` 分支经 jsDelivr 下载。**所以上架前必须先发语言包**,否则用户装上后下不到词典、无法翻译。
```bash
node  browser-extension/build-dict.mjs            # 由 glossary.*.json 生成 dict.*.json
bash  browser-extension/scripts/publish-langpacks.sh   # 推到 langpacks 分支(jsDelivr 几分钟生效)
node  browser-extension/scripts/pack-store.mjs    # 产出轻量上传包 dist/vcf-rosetta-<version>.zip
```
- 验证 CDN 可访问:`curl -I https://cdn.jsdelivr.net/gh/vcf-rosetta/vcf-rosetta@langpacks/dict.zh-CN.json` 返回 200。
