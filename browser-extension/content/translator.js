...
(function () {
  ...
  function applyPhrases(s) {
    ...
    // 廉价预筛:绝大多数模式要求串内含数字;其余仅少数固定前缀/关键短语。先挡掉,免去逐条正则。
    // 注意:曾有 bug —— 不含数字的任务串(如 "Query container volume async task running on
    // target vc.knight.com finished with status SUCCESS")被预筛拦下,永远到不了 PHRASES,
    // 现场单条采集量高达 1600+。凡新增"无数字也可能出现"的模式,必须同步登记到这里。
    if (!/\d/.test(s) &&
        !/task running on target|vCenter agent on|\(Maintenance Mode\)$|\(Not responding\)$|\(Disconnected\)$|\(static\)$/i.test(s) &&
        !/^(Updated|Last updated|Active sessions|Idle sessions|Ready to upgrade|a few seconds|used$|active$|\(disconnected\)|\(connected\)|Not running|Running|Processing data|Retrieving data|Imported from|Cannot synchronize|You selected to shut down|Certificate viewer:|Standard Switch:|Distributed Switch:|VMkernel network adapter:|Actions - |Waiting for operations|This vCenter Server is managed)/i.test(s)) return s;
