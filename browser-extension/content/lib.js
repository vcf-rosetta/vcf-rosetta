...
(function (root, factory) {
  ...
  const NOISE_RE = [
    ...
    // 实例/对象名:≥2 分隔符且含数字,无空格(如 knight-md-cl01)。
    // 段用 [a-z0-9](不含 _/-),分隔符用 [-_] —— 二者字符集不相交,匹配是确定性的,
    // 不会像旧写法 [\w]*([-_][\w]+){2,} 那样因 [\w] 覆盖 _ 与分隔符重叠而灾难性回溯(ReDoS)。
    /^(?=.*\d)[a-z0-9]+(?:[-_][a-z0-9]+){2,}$/i,
  ];
  ...
  function looksTranslatable(s) {
    ...
    // 防御性长度门(ReDoS 纵深):下方 NOISE_RE/DYNAMIC_RE 均是「无空格的短标识符」形态
    // (REST 路径 / 主机名 / 实例名),段落级长描述必带空格、绝不会命中它们。对「超长且无空格」
    // 的串直接判为机器标识,既避免把异常长 token 喂进正则,又不误伤真正的长描述(有空格,照常放行)。
    if (s.length > 200 && !/\s/.test(s)) return false;
