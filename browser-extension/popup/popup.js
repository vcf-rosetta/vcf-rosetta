...
function applyUi() {
  ...
  // 捐赠通道按界面语言切换:简体中文用微信赞赏码(PayPal 大陆不可用),其它语言用 PayPal
  const isCN = ui === 'zh-CN';
  const paypalEl = document.getElementById('donatePaypal');
  const wechatEl = document.getElementById('donateWechat');
  if (paypalEl) paypalEl.hidden = isCN;
  if (wechatEl) wechatEl.hidden = !isCN;
